...
HomTransform HomTransform::fromDH(double alpha, double theta, double a, double d){
    ...
    ht.setDH(alpha, theta, a, d);
    return ht;
 }
 
void HomTransform::updateFromDHTheta(double theta){
    Eigen::Matrix3f rot;
    double ct, st ,sa, ca;
    ct = cos(this->dhParams.theta);
    st = sin(this->dhParams.theta);
    sa = sin(this->dhParams.alpha);
    ca = cos(this->dhParams.alpha);
    
    rot << ct, -st*ca, st*sa,
           st, ct*ca, -ct*sa,
           0.0,  sa,     ca;
    Eigen::Vector3f trans;
    trans << this->dhParams.a*ct, this->dhParams.a*st, this->dhParams.d;
    
    this->setRotation(rot);
    this->setTranslation(trans);
    
}
void HomTransform::updateFromDH_D(double d){
    this->setTranslation(Eigen::Vector3f(this->v_translation(0), this->v_translation(1), d));
    std::cout << "updating: " << this->v_translation(0) << this->v_translation(1) <<  d << std::endl;
}

void HomTransform::setDH(double alpha, double theta, double a, double d){
    this->dhParams.a = a;
    this->dhParams.alpha = alpha;
    this->dhParams.theta = theta;
    this->dhParams.d = d;
}
...
void HomTransform::setRotationFromQuaternion(Eigen::Vector4f quat){
    if(( fabs(quat(0) - 1.0f) < NUM_TOL) && ( fabs(quat.norm() - 1.0f) < NUM_TOL )){
        this->setRotation(Eigen::Matrix3f::Identity());
        return;
    }
    double angle = acos(quat(0))*2.0;
    //double denom = 1.0/ (sin(angle/2));
    double denom = sqrt(1.0f - quat(0) * quat(0));
    //Eigen::Vector3f axis = denom *quat.bottomRows(3);
    Eigen::Vector3f axis = quat.bottomRows(3) / denom;
    this->setRotationFromAngleAxis(angle,axis);
}
/*Siciliano p.55 theorem 2.35*/
...
//    quaternion(0) = acos(angle/2.0);
//    return quaternion;
    Eigen::Vector4f quaternion;
    quaternion(0) = 0.5*sqrt(this->m_rotation(0,0) + this->m_rotation(1,1) + this->m_rotation(2,2) + 1.0f);
    quaternion(1) = 0.5*sqrt(this->m_rotation(0,0) - this->m_rotation(1,1) - this->m_rotation(2,2) + 1.0f);
    if(this->m_rotation(2,1) - this->m_rotation(1,2) < 0)
        quaternion(1) *= -1;
    quaternion(2) = 0.5*sqrt(this->m_rotation(1,1) - this->m_rotation(2,2) - this->m_rotation(0,0) + 1.0f);
    if(this->m_rotation(0,2) - this->m_rotation(2,0) < 0)
        quaternion(2) *= -1;
    quaternion(3) = 0.5*sqrt(this->m_rotation(2,2) - this->m_rotation(0,0) - this->m_rotation(1,1) + 1.0f);
    if(this->m_rotation(1,0) - this->m_rotation(0,1) < 0)
        quaternion(3) *= -1;
    return quaternion;
...
void HomTransform::setTranslationWithAxis(double _displacement, Eigen::Vector3f _axis){
    this->v_translation = _axis*_displacement;
    this->m_transform.topRightCorner(3,1) = this->v_translation;
}
...
void HomTransform::setName(std::string _name){
    this->name = _name;
}

void HomTransform::setChildName(std::string _cname){
    this->child = _cname;
}

std::string HomTransform::getName(){
    return this->name;
}

std::string HomTransform::getChildName(){
    return this->child;
}